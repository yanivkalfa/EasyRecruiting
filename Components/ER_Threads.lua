...
function EasyRecruiting.threads.createNewThread(name)
  return { 
    name = name,
    messages = {}
  };
...
function EasyRecruiting.threads.addThread(name)
  local thread = EasyRecruiting.threads.getThread(name);
  if ( thread ) then
    return thread, false;
  end
  
  local newThread = EasyRecruiting.threads.createNewThread(name);
...
function EasyRecruiting.threads.addMessageToThread(name, message)
  local thread, isNew = EasyRecruiting.threads.addThread(name);
